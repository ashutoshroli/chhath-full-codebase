...
export default function ReceiptTemplates() {
  ...
  const refreshList = () => {
    api.getReceiptTemplates().then(list => {
      setTemplates(list);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    api.getReceiptTemplate(year).then(row => {
  ...
  const save = async () => {
    ...
    setError('');
    try {
      await api.saveReceiptTemplate(year, text, pageSize);
      refreshList();
    } catch (err) {
  ...
  const createNew = async () => {
    ...
    setError('');
    try {
      await api.saveReceiptTemplate(newYear, text || '', 'A5');
  ...
  const doCopy = async () => {
    ...
    setError('');
    try {
      await api.copyReceiptTemplate(year, copyTarget);
  ...
  const remove = async () => {
    if (!confirm(`Delete the receipt template for Year ${year}?`)) return;
    try {
      await api.deleteReceiptTemplate(year);
      setYear(null);
      refreshList();
    } catch (err) {
      setError(err.message);
  ...
  return (
    <>
      <h2 style={{ marginBottom: 15 }}>Receipt Templates</h2>
