...
export default function SamaanTemplates() {
  ...
  const refreshList = () => {
    api.getSamaanTemplates().then(list => {
      setTemplates(list);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    api.getSamaanTemplate(year).then(row => {
  ...
  const save = async () => {
    ...
    setError('');
    try {
      await api.saveSamaanTemplate(year, text, pageSize);
      refreshList();
    } catch (err) {
  ...
  const createNew = async () => {
    ...
    setError('');
    try {
      await api.saveSamaanTemplate(newYear, text || '', 'A5');
  ...
  const doCopy = async () => {
    ...
    setError('');
    try {
      await api.copySamaanTemplate(year, copyTarget);
  ...
  const remove = async () => {
    if (!confirm(`Delete the Material template for Year ${year}?`)) return;
    try {
      await api.deleteSamaanTemplate(year);
      setYear(null);
      refreshList();
    } catch (err) {
      setError(err.message);
  ...
  return (
    <>
      <h2 style={{ marginBottom: 15 }}>Material Templates</h2>
