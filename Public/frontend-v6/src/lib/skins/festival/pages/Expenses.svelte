...
<h1 class="mb-4 text-xl font-black text-[#7a1420]">{$tr('expenses_ledger')}</h1>
{#if $portalState.failed}
  <ErrorState />
{:else}
  <div class="{CARD} mb-3 flex items-center justify-between p-4">
    <span class="text-sm font-semibold text-[#7a1420]/70">{$tr('total_expense')}</span>
    <strong class="text-xl text-[#B01E2E]">{fmt(total)}</strong>
  </div>
  <div class="{CARD} divide-y divide-[#F5B840]/30 p-4">
    {#if loading}
      {#each Array(6) as _}<div class="my-3 h-5 animate-pulse rounded bg-[#F5B840]/20"></div>{/each}
    {:else if items.length === 0}
      <p class="py-8 text-center text-sm text-[#7a1420]/60">{$tr('no_expenses')}</p>
    {:else}
      {#each items as e, i (i)}
        <div class="flex items-center justify-between gap-3 py-3">
          <div class="min-w-0">
            <div class="truncate font-medium text-[#7a1420]">{desc(e) || $tr('na')}</div>
            <div class="text-xs text-[#7a1420]/60">{#if e.category}{e.category} · {/if}{e.year}</div>
          </div>
          <strong class="flex-none text-[#B01E2E]">-{fmt(e.amount)}</strong>
        </div>
      {/each}
    {/if}
  </div>
