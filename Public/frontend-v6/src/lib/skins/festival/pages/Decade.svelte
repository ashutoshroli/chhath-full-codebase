...
<div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#7a1420] to-[#B01E2E] p-6 text-center text-[#ffe9c7] shadow-lg">
  ...
  <div class="relative">
    ...
    <p class="mt-2 text-xs font-bold text-[#F5B840]">{$tr('decade_years')}</p>
  ...
  <ol class="relative space-y-4 border-l-2 border-[#F5B840] pl-5">
    {#each DECADE_YEARS as y, i}
      <li class="relative">
        ...
        <h4 class="text-sm font-black text-[#B01E2E]">{$tr(y.headingKey)}</h4>
        <p class="mt-1 text-sm text-[#7a1420]">{$tr(y.bodyKey)}</p>
        {#if y.total}
          <div class="mt-2 flex flex-wrap gap-2">
            <span class="rounded-lg bg-emerald-600/10 px-3 py-1.5 text-xs font-bold text-emerald-800">{$tr('decade_total_label')}: {y.total}</span>
            <span class="rounded-lg bg-[#B01E2E]/10 px-3 py-1.5 text-xs font-bold text-[#B01E2E]">{$tr('decade_contributors_label')}: {y.contributors}</span>
          </div>
        {:else}
...
<!-- Table -->
<div class="{CARD} mt-4 p-5">
  <h3 class="text-base font-black text-[#5a0f18]">{$tr('decade_table_h')}</h3>
  <div class="mt-3">
    ...
    {#each DECADE_TABLE as r}
      <div class="grid grid-cols-[1fr_1.4fr_1fr] gap-2 border-b border-[#F5B840]/30 py-2 text-sm last:border-0">
        <span class="font-bold text-[#B01E2E]">{r.year}</span>
        <span class="font-semibold text-emerald-800">{r.total}</span>
        <span class="text-[#7a1420]">{r.contributors}</span>
      </div>
    {/each}
  </div>
  <div class="mt-4 rounded-xl bg-[#B01E2E]/10 p-4">
    <p class="text-xs font-extrabold uppercase text-[#B01E2E]">{$tr('decade_totals_h')}</p>
    <p class="mt-1 text-lg font-black text-[#5a0f18]">{$tr('decade_total_amount')}</p>
    <p class="text-sm font-bold text-[#7a1420]">{$tr('decade_total_entries')}</p>
    <p class="mt-2 text-xs text-[#7a1420]/70">{$tr('decade_total_clarify')}</p>
  </div>
</div>
