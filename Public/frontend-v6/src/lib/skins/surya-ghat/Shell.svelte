...
<div class="surya-shell min-h-screen">
  ...
  <header class="sticky top-0 z-40 border-b border-amber-300/15 bg-slate-950/75 backdrop-blur-xl">
    <div class="mx-auto flex max-w-6xl items-center gap-3 px-4 py-3 md:px-6">
      <a href="/" class="group flex min-w-0 items-center gap-3" aria-label={$tr('app_title')}>
        <span class="grid h-11 w-11 flex-none place-items-center rounded-2xl border border-amber-300/30 bg-gradient-to-br from-amber-300/20 to-teal-300/10 shadow-[0_0_35px_rgba(230,166,67,.12)]">
          <Sun class="h-6 w-6 text-amber-300 transition-transform group-hover:rotate-45" aria-hidden="true" />
        </span>
        <span class="min-w-0 leading-tight">
          <span class="block truncate text-sm font-black tracking-wide text-white">{$tr('app_title')}</span>
          <span class="flex items-center gap-1 text-[0.66rem] font-semibold uppercase tracking-[.18em] text-teal-200/70"><Waves class="h-3 w-3" />{$tr('app_subtitle')}</span>
        </span>
      </a>
  ...
  <StatusBanner />
  <main id="main" tabindex="-1" class="surya-main mx-auto px-4 pb-28 pt-6 md:px-6 md:pb-12">
    {@render children()}
  </main>

  <footer class="mx-auto max-w-6xl px-4 py-8 pb-28 text-center text-xs text-slate-400 md:px-6 md:pb-8">
    <p class="font-bold text-slate-300">{$tr('org_name')}, {$tr('org_location')}</p>
    <p class="mt-1 text-teal-200/60">{$tr('seva_line')}</p>
    <FooterLinks />
  </footer>
