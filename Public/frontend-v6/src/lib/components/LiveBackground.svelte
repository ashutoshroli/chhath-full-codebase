...
<div class="pointer-events-none fixed inset-0 -z-10 overflow-hidden" aria-hidden="true">
  <!-- Sky gradient: driven by the active theme's --page-* tokens so every theme
       (warm, slate, classic, midnight) gets its own backdrop. -->
  <div
    class="absolute inset-0"
    style="background-image: linear-gradient(to bottom, var(--page-from), var(--page-via), var(--page-to));"
  ></div>

  <!-- Sun / glow — only for themes that opt in (--page-sun: 1); flat themes
       (classic/slate) hide it via the CSS var. -->
  <div
