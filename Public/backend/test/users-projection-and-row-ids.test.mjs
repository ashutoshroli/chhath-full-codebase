...
// deliberately withheld, and `date_of_birth` standing in for whatever the table grows
// next — the column nobody reviewed, because it did not exist when this code was written.
const USER_ROW = {
  id: 41,
  id_code: 'USER0007',
  name: 'Amit Kumar',
...
describe('the public users payload is an allowlist (PUB-BE-05)', () => {
  ...
  test('a column the table grows LATER does not ship itself', async () => {
    const data = await portal(makeEnv());
    // THE finding: a denylist fails open. `date_of_birth` is on the row and on no
    // list — under the old code it was published the moment the column existed.
    assert.equal(data.users[0].date_of_birth, undefined);
    assert.ok(!JSON.stringify(data.users).includes('1990-04-12'),
