...
const app = {
  ...
  appendChatMsg: (text, kind) => {
    ...
    if (!wrap) return null;
    const baseCls = 'max-w-[85%] px-3 py-2 rounded-xl text-[0.86rem] leading-snug whitespace-pre-wrap break-words';
    const kindCls = kind === 'user'
      ? 'self-end rounded-br bg-saffron-light dark:bg-orange-900/40 text-orange-900 dark:text-orange-200'
      : kind === 'error'
        ? 'self-start rounded-bl bg-red-100 dark:bg-red-900/40 text-red-800 dark:text-red-300'
        : 'self-start rounded-bl bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-100';
  ...
  renderStaleNotice: (savedAt) => {
    ...
    bar.className = 'bg-amber-100 dark:bg-amber-900/40 text-amber-800 dark:text-amber-300 text-center text-[0.8rem] px-3 py-1.5 leading-relaxed';
  ...
  renderColdFailureFallback: () => {
    ...
    loader.innerHTML =
      '<div class="text-center p-6 max-w-[420px] mx-auto">' +
      '<img src="/logo.svg" alt="" width="72" height="72" class="w-[72px] h-[72px] mb-3 mx-auto">' +
      '<h2 class="mb-1.5 text-[1.05rem] font-semibold">' + escapeHtml(title) + '</h2>' +
      '<p class="text-gray-500 dark:text-gray-400 text-[0.9rem] mb-4">' + escapeHtml(msg) + '</p>' +
      '<button onclick="location.reload()" class="px-5 py-2.5 border-0 rounded-lg bg-saffron text-white font-semibold cursor-pointer">' + escapeHtml(retry) + '</button>' +
      '</div>';
  ...
  renderPopupSlide: () => {
    ...
    content.innerHTML = `
      ...
      ${slide.link_url && safeUrl(slide.link_url) ? `<a class="inline-block mx-4 mb-4 px-4 py-2 rounded-lg bg-saffron text-white no-underline font-semibold text-[0.85rem] [overflow-wrap:anywhere] max-w-[calc(100%-32px)]" href="${escapeHtml(slide.link_url)}" target="_blank" rel="noreferrer">${escapeHtml(slide.link_text || 'Learn more')}</a>` : ''}
    `;
  ...
  checkRecordVerification: () => {
    ...
    content.innerHTML = `
      <div class="flex items-center gap-2">
        <span class="material-icons-round ${genFile ? 'text-emerald-500' : 'text-red-500'}">${genFile ? 'verified' : 'error_outline'}</span>
        <strong>${genFile ? app.t('verified_record') : app.t('record_not_found')}</strong>
      </div>
      <div class="text-[0.85rem] text-gray-500 dark:text-gray-400 mt-1">${escapeHtml(docLabel)} — ${app.t('year')} ${escapeHtml(year)}</div>
      ${genFile ? detailsHtml : `<p class="text-[0.85rem] mt-2">${app.t('verify_help')}</p>`}
    `;
      ...
      .map(r => {
         ...
         if (app.isResellRow(r)) {
            return `<div class="${dataRowCls}">
               <div>
                 <strong class="block">♻️ ${app.t('resell')}: ${escapeHtml(r.Detail || '-')} ${yrTag}</strong>
                 <span class="text-[0.8rem] text-gray-500 dark:text-gray-400">${app.t('resold_item')}</span>
               </div>
         ...
         const rightSide = isMoney
            ...
            : `<div class="text-right">
                 <span class="px-2 py-1 rounded text-xs font-bold bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300">${cType === '2' ? app.t('material') : app.t('service')}</span>
         ...
         return `<div class="${dataRowCls}">
            <div>
              ...
              <span class="text-[0.8rem] text-gray-500 dark:text-gray-400">${escapeHtml(app.localize(u, 'Village') || r.Village || '-')} | ${escapeHtml(app.localize(u, "Father's Name") || '-')}</span>
            </div>

            ${rightSide}
         </div>`;
      }).join('');
    document.getElementById('home-col-list').innerHTML = html || `<div class="text-center py-5">${app.t('no_records_found')}</div>`;
  },
  ...
  renderExpenses: () => {
    const html = app.currentData.exp.map(r => {
      ...
      return `<div class="flex justify-between items-center gap-2.5 py-3 border-b border-dashed border-gray-100 dark:border-gray-700 last:border-b-0 [&>*]:min-w-0 [&>*:first-child]:[overflow-wrap:anywhere]">
        <div><strong class="block max-w-[200px]">${escapeHtml(desc)} ${yrTag}</strong></div>
        <strong class="text-red-500">-${fmt(r.Amount)}</strong>
      </div>`;
    }).join('');
    document.getElementById('exp-list').innerHTML = html || `<div class="text-center py-5">${app.t('no_expenses')}</div>`;
  },
  ...
  renderLoans: () => {
    ...
    if(targetLoans.length === 0) {
      document.getElementById('loans-dynamic-list').innerHTML = `<div class="bg-white dark:bg-gray-800 rounded-xl shadow-card border border-gray-100 dark:border-gray-700 text-center py-5">${app.t('not_distributed')}</div>`;
      return;
    }
    ...
    targetLoans.forEach(curLoan => {
      ...
      guars.forEach(row => {
         ...
         gHtml += `<div class="bg-white dark:bg-gray-800 rounded-xl shadow-card border border-gray-100 dark:border-gray-700 p-[15px] mb-2.5">
            <div class="flex justify-between items-center gap-2">
              <strong class="min-w-0 [overflow-wrap:anywhere]">${escapeHtml(app.localize(uGuarantor, 'Name'))}</strong>
              <span class="flex-shrink-0">${statusBadge}</span>
            </div>
      ...
      finalHtml += `
      <div class="rounded-xl shadow-card border border-amber-300 dark:border-amber-800 p-5 mb-4 bg-amber-50 dark:bg-amber-950/40">
        <h3 class="text-amber-800 dark:text-amber-300 mb-1.5 text-base font-semibold">${app.t('surplus_loan')} ${isAll ? `(${lYear})` : ''}</h3>
        <p class="text-[0.85rem] text-amber-700 dark:text-amber-400 mb-4">${app.t('given_to_verified')}</p>

        <div class="bg-white dark:bg-gray-800 p-[15px] rounded-lg border border-amber-200 dark:border-amber-800 mb-4">
          <div class="text-[0.8rem] text-gray-500 dark:text-gray-400">${app.t('receiver_name')}</div>
          <div class="text-[1.3rem] font-bold text-gray-800 dark:text-gray-100 mb-2.5">${escapeHtml(app.localize(uReceiver, 'Name'))}</div>
          <div class="grid grid-cols-3 gap-2.5">
            <div><span class="text-xs">${app.t('amount')}</span><br><strong class="text-[0.95rem]">${fmt(curLoan.Amount)}</strong></div>
            <div><span class="text-xs">${app.t('int_rate')}</span><br><strong class="text-[0.95rem]">${escapeHtml(curLoan['Intrest Rate'] || curLoan['Interest Rate'] || '0')}%</strong></div>
            <div><span class="text-xs">${app.t('tenure')}</span><br><strong class="text-[0.95rem]">${escapeHtml(curLoan.Tenure || '0')} Mo</strong></div>
          </div>
        </div>

        <h4 class="mb-2.5 text-amber-800 dark:text-amber-300 font-semibold">${app.t('verified_guarantors')}</h4>
       ...
       <div class="bg-white dark:bg-gray-800 rounded-xl shadow-card border border-gray-100 dark:border-gray-700 p-[15px] mb-3 flex gap-[15px] items-center">
          ...
          <div class="flex-grow min-w-0">
             <div class="flex justify-between items-start gap-2">
                <strong class="text-[1.05rem] min-w-0 [overflow-wrap:anywhere]">${escapeHtml(nameStr || '-')}</strong>
             ...
             <div class="text-[0.8rem] text-gray-500 dark:text-gray-400 flex flex-wrap gap-2.5">
                <span class="flex items-center gap-1"><span class="material-icons-round !text-xs">call</span> ${escapeHtml(u.Mobile || app.t('na'))}</span>
                <span class="flex items-center gap-1"><span class="material-icons-round !text-xs">place</span> ${escapeHtml(app.localize(u, 'Village') || app.t('na'))}</span>
             </div>
          </div>
       </div>`;
    }).join('');
  ...
  searchDownloadPeople: () => {
    ...
    const html = list.map(u => {
      ...
      <div class="flex justify-between items-center gap-2.5 py-3 border-b border-dashed border-gray-100 dark:border-gray-700 last:border-b-0 [&>*]:min-w-0 [&>*:first-child]:[overflow-wrap:anywhere] cursor-pointer" onclick="app.selectDownloadPerson('${escapeHtml(idJs)}')">
        <div>
          <strong class="block">${escapeHtml(app.localize(u, 'Name'))}</strong>
          <span class="text-[0.8rem] text-gray-500 dark:text-gray-400">${escapeHtml(app.localize(u, 'Village') || '-')}</span>
        </div>
        <span class="material-icons-round text-saffron">chevron_right</span>
      </div>`;
    }).join('');

    document.getElementById('dc-people-wrap').innerHTML = `<div class="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-card mb-4 border border-gray-100 dark:border-gray-700 [overflow-wrap:anywhere]" id="dc-people-list">${html || `<div class="text-center py-5">${app.t('no_matches')}</div>`}</div>`;
  },
  ...
  renderDownloadDocs: () => {
    ...
    const rowHtml = (item) => `
      <div class="flex justify-between items-center gap-2.5 py-3 border-b border-dashed border-gray-100 dark:border-gray-700 last:border-b-0 [&>*]:min-w-0 [&>*:first-child]:[overflow-wrap:anywhere]">
        <div><strong class="block text-[0.9rem]">${escapeHtml(item.label)}</strong></div>
        ${item.publicLink && safeUrl(item.publicLink)
          ? `<a href="${escapeHtml(item.publicLink)}" target="_blank" rel="noreferrer" class="px-2 py-1 rounded text-xs font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300 no-underline">${app.t('download')}</a>`
          : `<span class="px-2 py-1 rounded text-xs font-bold bg-gray-100 text-gray-400 dark:bg-gray-700 dark:text-gray-500">${app.t('not_available')}</span>`}
      </div>`;

    const section = (title, items) => `
      <h4 class="mt-4 mb-2 text-gray-800 dark:text-gray-100 font-semibold">${escapeHtml(title)}</h4>
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-card border border-gray-100 dark:border-gray-700 px-4 py-2.5">
        ${items.length ? items.map(rowHtml).join('') : `<div class="text-center py-2.5 text-gray-500 dark:text-gray-400 text-[0.85rem]">${app.t('no_docs')}</div>`}
      </div>`;

    wrap.innerHTML = `
      <div class="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-card mb-4 border border-gray-100 dark:border-gray-700 flex justify-between items-center gap-2.5">
        <div class="min-w-0 [overflow-wrap:anywhere]">
          <strong class="block text-[1.05rem]">${escapeHtml(app.localize(u, 'Name'))}</strong>
          <span class="text-[0.8rem] text-gray-500 dark:text-gray-400">${escapeHtml(app.localize(u, 'Village') || '-')}</span>
        </div>
        <button class="bg-gray-200 text-gray-900 dark:bg-gray-700 dark:text-gray-100 border-0 px-3.5 py-2 rounded-lg font-semibold text-[0.85rem] cursor-pointer flex-shrink-0" onclick="app.backToDownloadList()">${app.t('back')}</button>
      </div>
